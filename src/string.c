...
void push_str_to_memory(choi_asm *ca, Str str) {
  assert(ca->memory_size + str.size <= ca->memory_capacity);
  memcpy(ca->memory + ca->memory_size, str.data, str.size);

  ca->memory_size += str.size;
}
...
void parse_special_operands(Str ln, choi_asm *ca, size_t ip, Arena *strArena) {
  ...
  if (str_cmp(i_n, to_str("@proc"))) {
    ...
    if (*str_trims(ln).data == '"') {
      ...
      ca->program[ca->program_size++] = (instruction){.opcode = OP_PUSH_ADDR, .operand = {.as_int = ca->memory_size}, .arg1 = reg, .arg2 = 0};
